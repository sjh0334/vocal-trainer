...
- 多音旋律暂不作为 Primary Journey；首版只提供 A3 / C4 / E4 三个 8 秒单一长音目标。
- 目标音名和频率在选择、准备、实时练习页面持续可见；长音可在独立准备页试听。
- 进入练习准备页不请求麦克风、不自动倒计时；只有用户明确点击“开始练习”才进入音频会话。
- 示范音采用低增益三角波、适度低通和渐入渐出；试用发现 1400 Hz 低通偏闷后上移到 2400 Hz，并降低 Q 与峰值增益，在清晰度和不刺耳之间取平衡。
- 跑道改为 10 秒慢速窗口；显示使用 5 帧中值平滑，评分和保存的轨迹继续使用原始帧。
